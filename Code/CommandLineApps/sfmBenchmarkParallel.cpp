...
int main(int argc, char *argv[]) {
  ...
  // Set number of threads for loop
  ...
  vector<dir2d> Forces(peds.size());
  // Loop over time period
  while (curr_time < finish_time_s) {
    // Loop over each Pedestrian to get forces
    ...
    // Loop over each Pedestrian to update velocity and position
    #pragma omp parallel for
    for (int i = 0; i < peds.size(); i++) {
      // Calculate new velocity vector
      dir2d v_new = (peds[i] -> v) + Forces[i]*delta_t;
